<?php
class C_kerjasama_non_ppkg extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // Cek apakah pengguna memiliki role sebagai Non-PPKG
        $user = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        if (!$user || $user['role'] !== 'Non-PPKG') {
            $this->session->set_flashdata('error', 'Maaf, anda tidak bisa mengakses halaman Non-PPKG. Silahkan login dengan benar.');
            redirect('c_login'); // Redirect ke halaman login jika bukan Non-PPKG
        }
    }
    ...
    {
        $data1['title'] = "Non-PPKG - Daftar Event kerjasama";
        ...
        $this->load->view('non_ppkg/kerjasama_non_ppkg/kerjasama_non_ppkg', $data);
        $this->load->view('non_ppkg/_partials/footer');
    }

    public function create_event()
    {
        $data1['title'] = "Non-PPKG - Tambah Event";
        $data1['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();

        $this->load->view('non_ppkg/_partials/header', $data1);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/create_kerjasama');
        $this->load->view('non_ppkg/_partials/footer');
    }
    ...
    public function store_event()
    {
        ...
        if (!$this->upload->do_upload('avatar')) {
            ...
            $this->load->view('non_ppkg/kerjasama_non_ppkg/create_kerjasama_non_ppkg', $error);
        } else {
            ...
            redirect('non_ppkg/kerjasama_non_ppkg/c_kerjasama_non_ppkg/daftar_semua_event');
        }
    }

    public function edit_event($id_kerjasama)
    {
        $data1['title'] = "Non-PPKG - Edit Event";
        $data['kerjasama'] = $this->Kerjasama_model->get_event($id_kerjasama);
        $data1['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        // print_r($data);
        $this->load->view('non_ppkg/_partials/header', $data1);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/edit_event_kerjasama_non_ppkg', $data);
        $this->load->view('non_ppkg/_partials/footer');
    }
    ...
    public function create_peserta($id_kerjasama)
    {
        ...
        // print_r($datax);
        $this->load->view('non_ppkg/_partials/header', $data1);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/create_peserta_event_kerjasama', $data);
        $this->load->view('non_ppkg/_partials/footer');
    }
    ...
    public function store_peserta($id_kerjasama)
    {
        ...
        if (!$this->upload->do_upload('avatar')) {
            ...
            redirect(site_url('non_ppkg/kerjasama_non_ppkg/c_kerjasama_non_ppkg/daftar_semua_event'));
        }
    }
    ...
    public function daftar_semua_peserta($id_kerjasama)
    {
        ...
        // print_r($datax);
        $this->load->view('non_ppkg/_partials/header', $data1);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/data_peserta_event_kerjasama', $data);
        $this->load->view('non_ppkg/_partials/footer');
    }
    ...
    public function edit_peserta($id_registrasi)
    {
        ...
        //print_r($datax);
        $this->load->view('non_ppkg/_partials/header', $data1);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/edit_data_peserta_event_kerjasama', $data);
        $this->load->view('non_ppkg/_partials/footer');
    }
    ...
    public function update_peserta($id_kerjasama)
    {
        ...
        redirect(site_url('non_ppkg/kerjasama_non_ppkg/c_kerjasama_non_ppkg/daftar_semua_peserta/' . $this->input->post('id_kerjasama')));
    }
    ...
    public function delete_peserta($id_registrasi)
    {
        ...
        redirect(site_url('non_ppkg/kerjasama_non_ppkg/c_kerjasama_non_ppkg/daftar_semua_peserta/' . $data['x']->id_kerjasama));
    }

    public function daftar_semua_artikel()
    {
        $data['title'] = "Non-PPKG - Daftar Semua Artikel";
        $data1['artikel'] = $this->Kerjasama_model->get_all_artikel();
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $this->load->view('non_ppkg/_partials/header', $data);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/artikel_non_ppkg', $data1);
        $this->load->view('non_ppkg/_partials/footer');
    }

    public function create_artikel()
    {
        $data1['title'] = "Non-PPKG - Tambah Artikel";
        $data1['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $data['penulis'] = $this->m_data->getAll();
        $data['penulis1'] = $this->Kerjasama_model->get_penulis();
        // print_r($data12);
        $this->load->view('non_ppkg/_partials/header', $data1);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/create_artikel_kerjasama_non_ppkg', $data);
        $this->load->view('non_ppkg/_partials/footer');
    }
    ...
    public function store_artikel()
    {
        ...
        redirect('non_ppkg/kerjasama_non_ppkg/c_kerjasama_non_ppkg/daftar_semua_artikel');
    }

    public function edit_artikel($id_kerjasama)
    {
        $data1['title'] = "non_ppkg - Edit Artikel";
        $data['kerjasama'] = $this->Kerjasama_model->get_artikel($id_kerjasama);
        $data1['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        // print_r($data);
        $this->load->view('non_ppkg/_partials/header', $data1);
        $this->load->view('non_ppkg/kerjasama_non_ppkg/edit_artikel_kerjasama_non_ppkg', $data);
        $this->load->view('non_ppkg/_partials/footer');
    }
